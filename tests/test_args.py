...
@pytest.mark.parametrize(
    "args, expected_follow, expected_debug_command, expected_hardware",
    [
        ([], False, False, ["g213"]),
        (["--follow"], True, False, ["g213"]),
        (["--debug-command"], False, True, ["g213"]),
        (["--follow", "--debug-command"], True, True, ["g213"]),
        (["--hardware", "g213"], False, False, ["g213"]),
        (["--hardware", "g213", "g213"], False, False, ["g213", "g213"]),
    ],
)
def test_parser_various_args(
    args, expected_follow, expected_debug_command, expected_hardware
):
    parsed = parser.parse_args(args)
    assert parsed.follow == expected_follow
    assert parsed.debug_command == expected_debug_command
